...
export namespace Extensions {
  ...
  ): Promise<new (runtime?: Scratch) => ExtensionPlain> {
    ...
    function ExtensionConstructor(
      ...
      if (!runtime?.extensions?.unsandboxed && !ext.allowSandboxed) {
        throw new ExtensionLoadError(
          `FSExtension "${ext.id}" must be supported unsandboxed.`
        );
      }
      for (const i in ext.requires) {
        if (!Object.keys(context.EXTENSIONS).includes(i)) {
          throw new ExtensionLoadError(
            `FSExtension "${ext.id}" requires ${i} to be loaded.`
          );
        }
        if (
          Version.compare(context.EXTENSIONS[i], ext.requires[i]) ===
          ext.requires[i]
        ) {
          throw new ExtensionLoadError(
            `FSExtension "${ext.id}" requires ${i} to be at least ${ext.requires[i]}.`
          );
        }
      }
      ext.init(runtime);
        ...
        for (const argIndex in block.plainArguments) {
          ...
          const myRestConfig: DynamicArgConfigPlain = {
            afterArg:
              argIndex === "0"
                ? undefined
                : block.plainArguments[Number(argIndex) - 1].content,
            dynamicArgTypes: [],
            ...(arg.dyConfig ?? {}),
          };
          if (arg.dyConfig) {
            if (haveRest) {
              throw new GeneratedFailed(
                "Block can have only one rest argument."
              );
            } else {
              haveRest = true;
              if (Object.keys(ext.loadersWithDefault).includes(arg.inputType)) {
                (myRestConfig.dynamicArgTypes as AcceptedInputType[]).push(
                  (Object.keys(ext.loadersWithDefault).includes(arg.inputType)
                    ? "string"
                    : arg.inputType) as AcceptedInputType
                );
              } else {
                throw new GeneratedFailed(
                  "Rest argument must be string, number, bool or any loader."
                );
              }
            }
          }
      ...
      ext.blocks.forEach((block) => {
        ...
        const argList = block.plainArguments
          .map((arg) => {
            let needLoad = false;
            if (!AcceptedInputType.includes(arg.inputType)) {
              //不属于已知输入类型，需要loader
            ...
            if (needLoad)
              return {
                ...
                loader: Object.keys(ext.loadersWithDefault).includes(
                  arg.inputType
                )
                  ? ext.loadersWithDefault[arg.inputType]
                  : undefined,
                isRest: !!arg.dyConfig,
              };
        ...
        argList.forEach((arg) => {
          if (arg && !arg.loader /*没有找到匹配的loader*/) {
            throw new MissingError(
              `Cannot find valid arg loader of arg ${arg.name}`
            );
          }
        });
      ...
      /**
       * 在GandiIDE中，initExpandableBlocks会报错
       * TW不会
       * 这里临时注释掉，等待Rundll86原作者修复
       *
       * 2025-02-16
       * TODO绑定事项
       */
    ...
    return {
      ...
      to(...platforms: PlatformSupported[]) {
        for (const platform of platforms) {
          console.log(
            `Trying to load FSExtension "${objectPlain.id}" on platform "${platform}"...`
          );
